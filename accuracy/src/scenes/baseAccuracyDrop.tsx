...
export default makeScene2D(function* (view) {
  ...
  const dropGags = gags.filter((gag) => gag.GagType === "Drop");
  const GAG_IMG_DIR = "../../public/data/";

  for (let i = 0; i < dropGags.length; i++) {
    ...
    <>
      {pills.map((ref, i) => (
        <Pill
          ...
          y={700}
          radius={45}
          fill={CatppuccinColors.Lavender}
          text={dropGags[i].GagName}
          imgSrc={dropGags[i].Resource}
        />
      ))}
      <Txt
        ref={logo}
  ...
  yield * all (
    ...pills.map((p, i) => p().setText(String(dropGags[i].Accuracy) + "%", 150)),
  );
