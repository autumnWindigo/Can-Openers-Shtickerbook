...
export default makeScene2D(function* (view) {
  ...
  const squirtGags = gags.filter((gag) => gag.GagType === "Squirt");
  const GAG_IMG_DIR = "../../public/data/";

  for (let i = 0; i < squirtGags.length; i++) {
    ...
    <>
      {pills.map((ref, i) => (
        <Pill
          ...
          y={700}
          radius={45}
          fill={CatppuccinColors.Pink}
          text={squirtGags[i].GagName}
          imgSrc={squirtGags[i].Resource}
        />
      ))}
      <Txt
        ref={logo}
  ...
  yield * all (
    ...pills.map((p, i) => p().setText(String(squirtGags[i].Accuracy) + "%", 150)),
  );
  ...
  yield * all (
    ...
    title().position.x(-500, 0.5, easeOutCubic)
  )
})
